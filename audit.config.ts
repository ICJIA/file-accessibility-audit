...
export const ANNOUNCEMENTS = [
  {
    ...
    badge: "New",
    text: "Every report now ends with a checklist of what a person still needs to look at — including reports that score 100. Automated checks can confirm that accessibility structure is present; almost none of them can tell whether it is right. A picture described as \u201cimage\u201d passes the alt-text check and tells a screen reader nothing. A heading that describes the wrong section passes. So each check your document passed now comes with the one judgment the tool could not make, written as something to go and do, and the WCAG criteria this tool does not examine at all are listed by name with links rather than as bare numbers. Nothing in that list is a failure — it is the difference between a document that is structured correctly and one that actually reads well.",
  ...
  },
  {
    ...
    badge: "Important",
    text: "How scores and grades are calculated has changed, and many documents will score differently than before. Three things were corrected after readers told us the reports contradicted themselves. First, a document is no longer punished for being simple: checks that don't apply to it — table markup in a document with no tables, alt text in one with no images — now count as passing rather than being dropped, because a document with no tables does not have a table problem. A one-page notice and a longer agenda with the identical missing-title fault used to score 71 and 79; both now score 79. Second, a document's score can no longer rise above what its worst unresolved problem allows: a minor item holds it at 89, a moderate problem at 79, a critical problem at 69 — so a few perfect checks can no longer outweigh one serious failure. Third, the letter still comes from the same scale it always has (90 and above is an A, 80 a B, 70 a C, 60 a D, below that an F), so the number and the letter always agree. Two things are deliberately unchanged: a check the tool could not evaluate is left out rather than assumed to pass, and a scanned document still scores zero, because a screen reader can read nothing from it. Reports you shared before today show their corrected score when reopened, so an old link and a fresh audit of the same file agree.",
