...
export const ANNOUNCEMENTS = [
  {
    ...
    badge: "Important",
    text: "Scores are now capped by the worst problem in a document, and many documents will score and grade lower than before. A reader spotted that two reports with the same missing-title problem carried different grades — and they were right. The score averaged the checks that applied, so one fault counted for more in a short document with little to check, and a few perfect checks could outweigh one serious failure; two files missing both a title and a language declaration were scoring better than a file missing only the title. From now on a document's score cannot rise above what its worst unresolved problem allows: a minor item caps it at 89, a moderate problem at 79, and a critical problem at 69. The letter still comes from the same scale it always has — 90 and above is an A, 80 a B, 70 a C, 60 a D — so the number and the letter always agree. Scores only move down under this rule, never up. Where a score is being held at its ceiling the report names the problem holding it there, and a report you shared before today will show the corrected score when it is reopened, so an old link and a fresh audit of the same file agree.",
