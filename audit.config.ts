...
export const ANNOUNCEMENTS = [
  ...
  },
  {
    ...
    badge: "Verified",
    text: "How do we know this checker actually works? Fair question — running it on real documents only proves it agrees with itself. So we built eighteen small trap documents designed to fool it, each constructed so the correct answer is known in advance: a document with a perfect title that is secretly one big photograph, a picture description made only of blank spaces, internal structure that loops back on itself, headings that are really whole paragraphs, a form with no labels beside an identical one with proper labels, the leftovers design tools like Canva and InDesign leave behind, and text carrying hostile computer code to prove it stays harmless. The checker passed all eighteen then in the battery — after it caught the first of its own bugs, which is exactly what it is for: a description of only blank spaces was counted as a real description. That is fixed; a check now asks what a person would hear, not whether the field has something in it. No real document in our test set had the defect, so no existing score changes. The battery is rebuilt from scratch on every run and anyone can re-run it.",
    linkText: "How scoring works",
    linkTo: "/technical-details",
    /** In-app route — an ordinary router link. */
    linkExternal: false,
