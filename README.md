...
| `plausibleAnalytics.test.ts` | 3 | The Plausible snippet's three silent failure modes: `nuxt.config.ts` builds the script tag from the `ANALYTICS` constants rather than a hardcoded copy that could drift from the CSP; `data-domain` equals `DEPLOY.PRODUCTION_URL`'s hostname (the self-hosted instance discards events for any site it doesn't know); and the origin is https with no trailing slash, since it is used verbatim in both the CSP and the script URL |
