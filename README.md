...
**2,598 tests** across 169 test files (API 1408, Web 1141, CLI 49). Run all three suites with one summary:
...
| `AnnouncementBanner.test.ts` | 20 | The AnnouncementBanner component - permanent dismissal per announcement id, localStorage key scoping, re-show after clear, WCAG-version gating, the standing "What's New" heading that marks the banner as recent changes, and the wiring that cuts a long entry to its opening sentences: only those sentences render, a "Read the full update" link to `/announcements` appears **only** when something was actually cut (so it never promises text that does not exist), a space separates the text both from that link and from an entry's own link (Vue's condense mode drops whitespace-only text between elements, so the two underlined links rendered touching), and the archive link survives either way. One case runs against the real newest `ANNOUNCEMENTS` entry, so an entry rewritten as one enormous sentence shows up here |
