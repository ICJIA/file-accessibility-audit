...
| **31** | Matterhorn checkpoints disclosed | The landing page lists all 31 checkpoints of the [Matterhorn Protocol](https://pdfa.org/resource/the-matterhorn-protocol/) — the PDF Association's PDF/UA test model, the same one PAC implements — and which layer checks each: the audit engine, the [veraPDF](https://verapdf.org/) pass that runs alongside every PDF audit, or human review (47 of its 136 failure conditions are machine-uncheckable by any tool). When veraPDF cannot run, PDF reports say **"Did not run"** instead of hiding the panel (v1.91.0). |
...
The score itself carries the same warning visually, not just in prose further down the page. An amber **automation-limit band** — led by a solid amber header bar — sits directly under the verdict line in both views, *above* the fix-progress meter, so the reassuring parts cannot be reached without passing it — and travels with the score into the remediation before/after cards, the printable plan, the downloaded HTML report, and the share email. It appears for **any grade over a 79 — A and B, the grades that look done** (under the severity caps, 80+ means the worst automated finding is Minor or nothing, which is exactly when a reader closes the tab satisfied); a C/D/F report already leads with an action plan full of work — and since v1.73.1 those grades carry a compact one-line reminder in the same spot ("Whatever the grade, automated checks are only part of the job"), so no score display at any grade is silent about the human half; "Still worth checking by hand" additionally covers it in full at every score. One predicate (`shouldShowAutomationLimit`) gates every surface, so the rule cannot drift between them. It says what accessibility remediators keep having to explain: **even a high score is not a guarantee**. The band splits the job in two — *Automated checks: done, this is the score* beside *Human review: always still required* (drawn with an open dashed border, work that never closes automatically) — states that a high score means the document handles the signals automated tests can measure, **not** that it is guaranteed to work with a screen reader, and reports the real count of WCAG criteria the audit never machine-checked at all. Deliberately no percentage: any figure out of 100 beside a letter grade gets read as the grade (learned on v1.58.1), and a canned "automated tools catch only N% of issues" statistic would be someone else's number about someone else's tool. The plain counts are this tool's own.
...
**2,650 tests** across 171 test files (API 1437, Web 1164, CLI 49). Run all three suites with one summary:
...
<details>
<summary><strong>Per-file coverage</strong> — 84 test files — click to expand</summary>
...
<details>
<summary><strong>Per-file coverage</strong> — 81 test files — click to expand</summary>
...
| `matterhornChecklist.test.ts` | 18 | The landing page's Matterhorn Protocol coverage disclosure (v1.91.0; plain-language rewrite v1.92.0): exactly the protocol's 31 checkpoints in order with the 87+47+2=136 arithmetic; the three non-technical FAQ cards (why "Matterhorn" — the Alpine mountain and the PDF Association; what veraPDF is — an independent second opinion that runs automatically; and a plain "No" to "does this change my score?"); **overclaim guards** — the human-judgment checkpoints (03 Flicker, 04 Color and Contrast, 22 Article Threads) must never claim machine coverage, Character Mappings (10) stays veraPDF-attributed until the in-house glyph census ships, and the v1.92.0 promotions are pinned exactly (17/19 engine-partial, 20 engine); the flagship in-house checkpoints (26 Security, 31 Fonts) stay engine-attributed; component renders every checkpoint with its coverage chip, the four-mechanism legend, the "Did not run" cross-reference, and noopener external links; and index.vue wiring |
...
<details>
<summary><strong>Earlier per-release reviews</strong> (v1.91.0 → v1.33.0) — click to expand</summary>
