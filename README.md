...
Every report opens in the **Visual view**: the grade, then a numbered action plan that walks through one fix at a time in plain language, with instructions for both the source document and Adobe Acrobat. Fix steps are written for the current app versions (Microsoft 365 Word and Acrobat's 2023+ "All tools" interface, verified August 2026), give the older classic-Acrobat path in parentheses where it differs, and every fix card says so — including how to tell which Acrobat you're on and who to contact (IDS at ICJIA) if the menus don't match. The **Detailed view** holds the complete technical report — every finding, the WCAG criteria it maps to, the evidence, PDF/UA signals, methodology.
...
**2,260 tests** across 138 test files (API 1177, Web 1034, CLI 49). Run all three suites with one summary:
...
| `reportVisualView.test.ts` | 7 | The assembled Visual view: zone DOM order (hero → tiles → verdict → plan → bars → technical report), plan steps built from the result, the failure-mode plan copy wired through (a fonts-only Text Extractability flag renders the font-embedding step, not the scanned-document copy), warnings + notice slot, evidence clicks opening the technical expander, the page-audit guard (hero only — never tiles/plan/bars/expander/pass-card), and legacy strict-profile derivation matching the Detailed view |
