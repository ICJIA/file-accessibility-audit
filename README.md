...
The cap **only ever lowers** a grade, never raises one, so a poor average keeps its worse letter. Where the letter ends up below what the score alone would give, both report views say so explicitly ("Held at C by a moderate issue — the 87 average alone would be a B"), because an unexplained C beside an 87 reads as a bug. Rule and helpers live in `packages/shared/src/scoring.ts` (`SEVERITY_GRADE_CAPS`, `capGradeBySeverity`, `gradeCapReason`) so the analyzer, web, API, and CLI share one definition.
