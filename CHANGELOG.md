...
- **"No heading tags" is scored on evidence, never on length.** A PDF with no `<H1>`–`<H6>` tags used to be a confirmed WCAG 1.3.1 failure — "its sections exist only visually" — whenever it had four or more pages, twenty or more paragraphs, or a single bookmark, without the checker ever seeing a visual heading; a one-page funding chart with one bookmark and two-page fact sheets in the reference corpus were accused that way. A new visual-heading census (`visualHeadings.ts`) now looks at the painted text: a line uniformly larger than the body size, or uniformly bold at it, short, contiguous (a row of cells with wide gaps is a table, not a heading), and sitting over a body-size line on the same page. Two or more such lines is sections and the report names them; one is a title; none is a document with no visual structure to convey, which is not scored on headings at all. The one-page memo with two bold subheads over body text IS scored now — length was never the test. Three real documents and three traps moved; the twelve-page traps that asserted the old proxy now paint real visual section titles, and three new traps pin the converse (plain prose with no visual headings, a descriptive title with an export timestamp, a one-page chart with one bookmark). Every threshold is published on /technical-details.
- **A file-name title is WCAG failure F25 only when it cannot describe the document.** F25's own examples are authoring-tool defaults, placeholders, and "filenames that are not descriptive in their own right". The old heuristic condemned "Lewd Sexual Display in Prison 2024 Annual Report 1-26-25-250127T16462808" (an export timestamp) and "Full_Report_Statewide_Violence_Prevention_Plan_2025-2029_2025_Update" (underscores) as confirmed Level A failures — titles that plainly identify their documents. Three shapes now: a bare file name, tool default, placeholder or pure timestamp/hash is scored and asserted as F25; a title that merely looks like a file name but carries at least two real words earns full credit and an unscored advisory (whether it describes the document *well* is a person's judgment — a new "Descriptive document title" best-practice row carries it); everything else is descriptive. Eight remediated corpus files regained the title credit they had been docked.
- **A text box exported as a `<Figure>` is not "missing alt text".** A screen reader reads the text inside a figure that has no `/Alt` — adding alt would hide it — so the report already told authors to retag rather than describe. It nonetheless counted those figures in the coverage score and the 1.1.1 failure. They are excluded from both now (the retagging advisory stays, and appears even when every other image is described); five real reports gained between two and ten alt-text points.
- **Word: the fake-heading rule looks only where a heading could be.** A bold 14-pt line inside a data-shaped table (two or more rows and columns — a header cell) or a text box (a callout) is no longer a confirmed 1.3.1/F2 failure; a title in a one-row banner table still is, and "Title"/"Subtitle"-styled paragraphs are structural. **Large text is resolved through the style**: a 16-pt bold Heading 1 whose size lives in the style and whose run only carries a grey colour is held to 3:1, not 4.5:1. **A linked picture takes its name from its alt text** — a linked logo with a description is no longer a 4.1.2 "link with no name"; a linked picture without one still is.
- **2.4.6 Headings and Labels is disclosed, never claimed.** It was listed as a criterion the score evaluates for heading structure, slide titles and sheet names, while no gate could ever assert it (Understanding 2.4.6: it "does not require headings or labels"; whether they describe their content is a human judgment). Removed from those maps and added to every verdict's not-assessed list. Link quality now names the 1.3.1 it asserts for untagged links; sheet names carry no criterion; the Office-only categories have reference blocks; and a test holds the web and shared category maps identical. The card sentence "This score is tied to the following criteria" now says what is true: the criteria the category checks against, of which only a confirmed failure in the verdict moves the score.
- **Best practices.** "Document title shown in viewers" no longer renders as extra credit on a report where the flag earned title points (it has been scored since 2026-09-01; only the flag-off case was diverted). Two rows added: "Descriptive document title" (the F25 advisory above) and "Outline starts at H1" for PDF (PDF/UA 7.4.2 — the step-by-step level check reads H2 → H3 → H2 as sound). "Also noted in this report" now works per line, not per category, so an advisory no row reads can no longer vanish behind a category some other row happens to cover. One defect, one chip: a mixed heading convention and a workbook with no defined table each read WORTH DOING once. The complex-table association row defers to the score (association *is* the scored 1.3.1 condition). Labels no longer assert reviews that never happened; rows whose basis is Microsoft's own guidance link to it; the raw-URL rows hedge 2.4.4 the same way in every field.
- **Not-assessed disclosures corrected.** 3.3.8 Accessible Authentication is no longer listed on form PDFs (a form is not a login); sound-only annotations are disclosed under 1.2.1, video under 1.2.2; the encryption-denies-assistive-technology failure states on the page why it is asserted under 1.1.1 (to assistive technology the whole document is non-text content with no alternative — no criterion names the flag).

### Fixed

- **A clean category was told veraPDF had confirmed its finding.** The co-sign card on the detailed view branched on whether the category had a severity — and a perfect 100 carries the string "No issues found", so every clean card with a mapped veraPDF rule read "Independently confirmed — it failed the same point". It now branches on whether the category actually lost points; the guard test's fixture used a severity the analyzer never emits, and was corrected to the real one.
- **A veraPDF error was recorded as a conformance failure on the remediation page.** A timed-out or unparseable run arrives as `available:true, passed:false, error:…`; the job recorded `verapdf_failed`, stored `verapdf_passed = 0`, and the results page showed an amber "0 rule failures". Three states now: `verapdf_error` with a NULL verdict, a badge that says "could not be checked", and an AI-summary line that says the same instead of "FAILED".
- **Three surfaces said the opposite of what the gate does.** /technical-details called DisplayDocTitle "reported, never scored" (scored since v1.153); llms-full.txt said an unusable language value is "never asserted as a 3.1.1 failure" (it is, twice); the README called a two-axis table without `/Scope` "never a confirmed WCAG failure" (rule 7c asserts 1.3.1 for exactly that shape). All three corrected, with guard tests; the 2.2 form-criteria wording on /wcag-2-2 and in the v1.145 log entry corrected; "compliance benchmark for publication" (×4) replaced by "the verdict is the compliance answer"; Section 508 removed from the standards lists and the rebrand templates; /technical-details gained a "Thresholds and heuristics" list and a "How fix times are estimated" block so every number a report turns on is reproducible.
- **veraPDF routing and disclosure.** Only the ToUnicode rule corroborates text extraction — embedding and glyph rules were removed from scoring in v1.131 and had no business co-signing a step about untagged text. The Matterhorn projection routes the PDF/UA-1 clause 7.2 table and list rules to Tables and Lists (they sat in "Other PDF/UA rules" while those rows read clean), sends the RoleMap rules to Role Mapping, and leaves clause 6 unmapped rather than filing MarkInfo under Metadata. The parser counts a summary with no failure figure as zero, never one, and reports a compliant-but-errored run as an error. When veraPDF's WCAG-profile pass flagged contrast, the manual-review row for 1.4.3 says so with the count, and the strip's clean line says "by this checker".

### Security

- **Page-audit SSRF: resolve once, connect there.** `resolvePublicIp` judges every address a name resolves to (a name publishing [public, 127.0.0.1] passed on the first record); each page audit launches its own Chromium with the validated document host pinned into the browser's resolver (`--host-resolver-rules`), so the browser connects to the address the check approved — the same resolve-then-dial guarantee the PDF path has had; the per-page resolver cache that widened the rebinding window is gone and every request is re-checked. `/api/bulk-from-inventory`, an unauthenticated request that fans out into up to 100 fetches and analyses, has its own limiter (3 per hour per IP) and no longer claims authentication it does not have. pdf.js runs with `isEvalSupported: false` and a bounded `maxImageSize` (it is the one parser that runs in-process). Audit temp files are written owner-only like the remediation outputs; attacker-influenced strings are stripped of control characters before they reach the error log; the `x-powered-by` fingerprint is gone; stored page reports have their `helpUrl`s neutralised at rest as help links already were. `pnpm audit --prod`: 0 advisories. No new attack surface.

Tests 3,700 across 212 files · traps 156 (113 PDF + 43 Office) · ledger 286 rows · catalog 41 practices · all corpus gates green.
...
- **A white slide title on a dark banner was read as white-on-white.** The PowerPoint contrast check resolved a text run's background as the shape's own fill, or else the slide's background — ignoring anything stacked between. A title on a solid banner shape over a white slide was measured against the white and confirmed as a Level AA failure at 1:1, in three of the seven real decks in the reference corpus. The check now walks the shapes beneath the text in z-order: a solid shape fully containing the run is its background; a gradient, picture, or partial overlap makes the run unresolved rather than a guess. One real deck moved 89/B to 100/A; seventeen impossible same-colour findings disappeared.
- **The manual-review card asserted things nobody had checked.** Because bookmarks and reading order stopped being scored, those categories always sit at 100 — and the card treated 100 as "verified", printing "the document has bookmarks for navigation" on documents with none (the best-practices row a few inches above said so). The card now speaks only where the checks found nothing at all and no advisory is open, and its wording is format-aware.
- **veraPDF explanations are routed by rule id, never by keywords.** The figure-alt rule's description mentions a table in ISO's numbering, so its fix advice talked about table tags; the annotation rule's advice talked about form tooltips. Every rule in both 1.30.1 profiles is now mapped by id, with a deliberately conservative fallback for rules that ship later.
- **Fix steps cite the criteria that actually failed.** Plan-step chips, the printable plan and the JSON export used to print the category's whole criteria list — so a "no headings" step showed 2.4.6, a criterion whose own Understanding document says it does not require headings. They now cite the conformance verdict's attribution, and the verdict strip counts distinct criteria instead of raw failures. The bookmarks card cites nothing, because no WCAG criterion requires bookmarks.
- **Seven ways a score could move without naming a law, closed.** A tagged PDF with one to forty-nine characters of text, a short document whose only heading hid behind a role map, an outline dictionary with no entries, a hand-typed PowerPoint list, a single-column spreadsheet table, an unreadable Word metadata part, a half-failed analysis — each could deduct points with no failing criterion attributed. Each is now either not assessed, an advisory, or carries its criterion. The legal-basis gate now also checks the converse — a criterion the verdict names must correspond to a deduction — and its first run found eleven real documents where the link census and the criterion disagreed.
- **A real advisory could vanish between two labels.** A PDF with a single-column layout table beside genuine data tables matched the wrong sentence in the best-practices catalog, marking three table practices "not applicable" and dropping them from the report entirely. The match is now anchored to the only wording that means what it says. Office documents also gained the descriptive-link practice PDF already had, so a page of "click here" links is no longer answered with "link text is not a raw URL — met".
- **Titles that live only in the document's XMP metadata are read.** PDF 2.0 deprecates the older Info dictionary, and some tools write the title only in XMP — those documents were told they had no title at all, a confirmed 2.4.2 failure that was not true. The Info dictionary still wins when both exist, and an empty XMP title element is never mistaken for the markup that follows it.
- **Link text the tool cannot see is no longer judged.** Rotated text and image-only links defeat the geometry that pairs a link with its words; those links were falling back to their URL and being graded on it. They are now flagged for hand-checking, plainly labelled, and never scored.
- **One typed "1." does not make a list.** A lone paragraph starting with an enumerator was scored as a broken list — a Critical that capped the grade at D. A typed enumerator now counts only when there is list evidence nearby: another typed item, or a real list, within two non-empty paragraphs.
- **The Matterhorn checklist claimed veraPDF checks three things it cannot.** Checkpoints 05 (Sound), 24 (Non-Interactive Forms) and 29 (Actions) were labelled "checked by veraPDF as part of every PDF audit" — but veraPDF's PDF/UA-1 profile contains no rules for them, so that promise could never fire, and a report rendered them as machine-clean. All three are now honestly "human review", on the landing page and in every report.
- **The numbers in the README matched an older product.** A documentation audit read every public claim against the shipped behavior: a worked example whose arithmetic used two scores the scorer cannot produce, a note naming the wrong upload limit, a sample response showing two profiles diverging when one has been an alias of the other since v1.21, the wrong Matterhorn number for mixed heading conventions, one page still missing the ADA rule's 2028 tier, fifty-one stale per-file test counts and thirty-one test files that had never been listed at all. All corrected, and the environment-variable direction for choosing the displayed WCAG version now reads the way the code does.
- **Yesterday's absolute sentences got their boundaries.** The strip that says only WCAG 2.1 is counted no longer renders over stored reports graded before that was true; the AI export no longer says "no remediation is needed" when a Level A criterion is failing on a document whose grade merely survived it.

### Added

- **A title nobody hears is half a title.** A PDF can set a title and still tell every viewer to announce the filename instead — that is what an unset DisplayDocTitle viewer preference does, and W3C's own technique for 2.4.2 in PDF (PDF18) is setting it. A set title with the preference off now earns 25 of the title's 50 points instead of riding along free, the finding names the flag, the consequence and the Acrobat fix, and the old best-practice row moved to "Counted in your score". Calibration before shipping: 43 of the 47 titled real documents in the reference corpus already set the flag.
- **Links with no text at all are scored.** A link that reaches a screen reader with no name to announce is a 4.1.2 Name, Role, Value failure (Level A) — now counted, and reachable only when the tool genuinely attributed the link's text (see the hand-checking advisory above for when it cannot).
- **The adversarial corpus grew to 153 traps** (110 PDF, 43 Office): the stacked-banner contrast pair, the hand-typed PowerPoint bullet pair, a hard check on the title-display trap, and the clean-with-best-practice-debt control rebuilt around two debts now that the third is scored. The pinned score ledger covers 283 rows, and the accuracy-gate table in the README now tells the truth about which of the ten gates run in CI.

### Changed

- **Two real documents' stored grades moved with the DisplayDocTitle change**, exactly as re-blessed in the ledger: a 2009 annual report 43 to 39 (its set-but-hidden title is now Critical), and a 2023 report's title category 100 to 75 beneath its existing grade cap. Every other movement in this release is a label, not a number.
