...
**Gate rules (spec §Conformance gates — fire only on these):** 1.1.1 non-decorative images without alt; 2.4.2 no core title; 3.1.1 `metadata.language` null; 1.3.1 data tables (≥2×2) without header row; 1.4.3 `contrast.failing` non-empty. **notAssessed always:** 1.3.2 (reading order beyond the title-first check); plus 1.4.3 when `checkedRuns === 0` (docx wording); plus `{ sc: "1.2.2", name: "Captions (Prerecorded)", level: "A" }` when `hasMedia` (reason: embedded audio/video captions are not machine-verified). Add `"1.2.2": "captions-prerecorded"` to `WCAG_UNDERSTANDING_SLUGS`. Fix steps in `issue` strings use PowerPoint paths ("In PowerPoint: right-click the image → View Alt Text…", "Insert → Header & Footer is not a slide title — use a Title layout placeholder", "Review → Language", "Table Design → check Header Row"). **Shared tail (2026-07-02 pre-flight decision, supersedes the earlier duplication note):** first extract the docx gate's status/headline tail (from `const status: ConformanceVerdict["status"] = …` through the `return { status, failures, notAssessed, headline }`) into a file-local helper `finalizeVerdict(failures: ConformanceFinding[], notAssessed: NotAssessedCriterion[], contrastNotEvaluated: boolean): ConformanceVerdict` — the docx gate's tail becomes `return finalizeVerdict(failures, notAssessed, analysis.contrast.checkedRuns === 0)` and `evaluateDocxConformance`'s behavior stays pinned by the unchanged docxConformance tests. The pptx gate (and Phase 3's xlsx gate) then call the same helper.
