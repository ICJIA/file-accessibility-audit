...
const src = computed(() => {
  const entry = Object.entries(urls).find(([path]) => path.endsWith(`/${props.name}.svg`));
...
<template>
  <figure class="rounded-xl border border-[var(--border)] bg-[var(--surface-deep)] overflow-hidden">
    <figcaption
      v-if="title"
