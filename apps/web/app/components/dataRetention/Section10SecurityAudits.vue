<template>
  ...
  <section id="security-audits" class="scroll-mt-8">
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.55.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-07</strong> · scope: the public status page's presentation — not
          a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The status page's browser view now presents its sections as collapsible cards with an
        always-visible health summary on top, so a first-time visitor sees the essentials at a
        glance rather than a wall of tables. Presentation only: the page publishes exactly the same
        aggregate operational numbers as before (service health, engine availability, anonymous
        document counts and grades, backup recency), still contains no names, no filenames, and no
        content from anyone's documents, and still runs no scripts in your browser. What monitoring
        services read is byte-for-byte unchanged.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.48.1</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-05</strong> · scope: corrections to this policy and the
          explanatory pages — not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        Every explanatory page — this policy, the technical and scoring explanations, and the
        project documentation — was checked line by line against the source code, and everything
        found inaccurate was corrected. The corrections that matter for a reader of this policy: it
        now states plainly that the usage log records the caller's IP address and browser
        identification with each entry (it always has; § 8 previously implied otherwise), that
        usage-log entries are deleted after 365 days rather than kept indefinitely (§ 7 — the
        automatic deletion has existed since tool v1.20.1), and that refused uploads are recorded
        (§§ 2, 7, 8). The audit entries above for v1.43.0 through v1.48.0 were added in the same
        pass; this register had fallen six releases behind. The full list of corrections is in this
        policy's own change log (§ 14, policy v1.2).
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        One correction was visible on reports: the legend explaining severity colors described a
        score of 90–99 as meeting accessibility standards, while the scoring itself reserves that
        description for a perfect 100. The legend now matches the scorer.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept — only to how
          accurately these pages describe it.</strong
        >
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.48.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-08-05</strong> · scope: a full red/blue team audit of the entire
          application, plus a live check of the production web server — a standalone comprehensive
          review, not a per-release one.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        The whole tool was re-examined end to end: how files are received and refused, sign-in, the
        checking engines, the database, rate limits, what the service publishes about itself, and
        the web-server configuration in front of the application. The attack half of the review —
        poisoned archives, oversized files, forged web addresses, a dozen ways of trying to make the
        server fetch its own internal resources — was run against an isolated copy of the service,
        so no production data or statistics were touched; the live site received only read-only
        checks. Every attack was turned away.
        <strong
          >Nothing critical or serious was found. Two low-severity issues were identified and fixed
          the same day.</strong
        >
      </p>
        ...
        <li>
          <strong
            ><span
              ...
              >Fixed</span
            >
            Refused uploads no longer store the file name exactly as sent</strong
          >
          — When the tool refuses a file it keeps a note of the attempt, including the file name it
          was offered under. That name was being stored exactly as the sender supplied it — any
          length, any characters. Only an administrator can see those notes, and the screens that
          display them already refuse to treat stored text as anything but text, so this was not
          exploitable — but it left a single safeguard doing all the work. File names are now
          cleaned and length-limited <em>before</em> they are stored, on every path that writes
          them, so no future change can quietly reopen the gap.
        </li>
        <li>
          <strong
            ><span
              ...
              >Fixed</span
            >
            Line breaks can no longer hide inside stored file names</strong
          >
          — The cleaning rule that strips unusual characters from file names treated line breaks as
          ordinary spacing, so a name containing them passed through — and this affected normal,
          accepted uploads too, not just refused ones. A file name is a single line by definition,
          and anything that later prints these records one per line would have inherited the break.
          All spacing characters are now collapsed to a plain space before the rule runs.
        </li>
        <li>
          <strong
            ><span
              ...
              >Hardened</span
            >
            The production web server's protective headers were tightened</strong
          >
          — Three gaps were found in the web server that sits in front of the application (not in
          the application itself), and all three were fixed and verified on the live site the same
          day: browsers are now told to always use the encrypted connection on every page, not only
          on API responses; a duplicated, conflicting anti-embedding header was reduced to one clean
          value; and the protective headers now appear on error pages as well as successful ones.
        </li>
      </ul>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong>No change to what data is collected, how it is used, or how long it is kept</strong>
        — file names are simply cleaned more strictly before storage. Everything else examined —
        sign-in and its fail-safe behavior, the randomness behind codes and links, database query
        construction, how helper programs are launched, the guard against fetching internal
        addresses, and the encryption settings — was confirmed sound with no change needed. The full
        technical write-up, including the complete list of what was checked and found sound, is in
        the project's README security section.
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.47.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-05</strong> · scope: clearer labels on the public status page —
          not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        Two different columns on the public status page were both labeled "other" and meant opposite
        things — one counted documents that were audited normally but whose file type could not be
        told from their web address, the other counted uploads that were refused outright. A reader
        pointed out that the page appeared to contradict itself. The first is now labeled
        "Unrecognized extension", the second "Other file types", and a short section explains the
        difference. The counts themselves are unchanged, and the page still publishes only totals —
        no file names, no email addresses, nothing identifying anyone.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.46.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-04</strong> · scope: the public status page now counts refused
          uploads — this adds one new kind of record to the usage log.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        When someone offers the tool a file it cannot check — an old-format Word document, an image,
        a spreadsheet export — the tool previously kept no record of the attempt at all. It now
        records that a refusal happened, the file name the upload was offered under, and when. The
        file itself is never accepted, so <strong>no file content is ever stored</strong>
        — and, deliberately, no content fingerprint is recorded either, so a refused file can never
        be mistaken for an audited one by the safeguard that gates the remediation feature. The
        status page publishes only the resulting totals, grouped by file type.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong
          >This adds one item to what is collected: the name and time of a refused upload.</strong
        >
        These records are kept for the same 365-day period as the rest of the usage log and are
        visible only to an administrator; how data is used, and every other retention period, is
        unchanged.
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.45.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-04</strong> · scope: clearer refusal messages for old Office
          formats and CSV files — not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        Old-format Office files (.doc, .xls, .ppt, .rtf) are now refused with an explanation of why
        they cannot be checked and how to convert them, instead of a generic list of accepted
        formats; CSV files are told there is nothing to check and that this is not a fault. To give
        the right message even when a file has been renamed, the tool now recognizes these formats
        by their content — reading only a small, fixed-size portion of the file's beginning,
        deliberately without parsing the file itself, so a hostile file gains nothing. No new format
        is accepted, refused files are still never stored, and no response the server gives changed
        shape — only the wording.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.44.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-04</strong> · scope: letter-grade totals on the public status
          page — not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The public status page now shows how audited documents have scored, as counts of letter
        grades over three time windows. These are the same kind of anonymous totals the page already
        published: no file names, email addresses, or anything identifying a person or document
        participates — only counts. Because a figure like "62% scored F" invites being quoted as a
        statistic about all government documents, which it is not — people tend to check documents
        they already suspect have problems — the page prints that caveat
        <em>above</em> the numbers, and an automated test keeps it there.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.43.0</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-04</strong> · scope: two browser conveniences — not a security
          release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The status page gained a link back to the audit tool, and the tool now asks for confirmation
        before you leave the page while an audit is still running, so work in progress is not lost
        to a stray click. The confirmation uses the browser's own built-in dialog — no custom pop-up
        to get wrong for screen-reader users. Both changes live entirely in the visitor's browser;
        nothing new is sent to or stored on the server.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: presentation of the public status page — not
          a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The public status page is now legible in an ordinary browser: the same information, laid out
        as a colour-coded outline rather than one unbroken block of text.
        <strong>What it publishes has not changed</strong> — still only totals and yes/no answers,
        with no file names, no email addresses, and nothing identifying anyone who has used the
        service. The checks that enforce that were left in place and still pass. Two details of
        note: the page runs no scripts at all, so there is nothing on it that could execute in a
        visitor's browser; and everything it displays is escaped before rendering, so no value drawn
        from the system can be interpreted as page markup.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.41.1</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: an internal deployment script — not a
          security release.
        </span>
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The automatic checks introduced in the previous release ran a moment too early and reported
        the service as unavailable when it was in fact starting normally. They now wait for the
        service to respond before checking. This affected only the report printed during deployment;
        the service itself was healthy throughout, which was confirmed independently at the time.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: navigation and an accessibility fix — not a
          security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        The header now carries a "Status" link and no longer carries an "Analyze" link — clicking
        the site title clears your results and starts a new file, which it already did. Because that
        title is now the only way to start over, it was also fixed to work with a keyboard:
        previously it responded only to a mouse click, which made it unusable for anyone navigating
        by keyboard or screen reader. On a tool that audits other people's documents for exactly
        this kind of problem, that was worth correcting promptly.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        This release also adds automatic checks that run after each deployment, confirming the
        service answers correctly and that its search-engine instructions file is being served. That
        file is currently <em>not</em> being served in production due to a web-server configuration
        issue unrelated to this application's code; the pages that most need to stay out of search
        results carry a separate instruction in their response headers, which is working. The
        configuration fix is queued.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        not respond to one of the two standard ways of asking. A monitor set up that way would have
        reported the service as down while it was working normally — a false alarm that, if
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.40.2</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: where one link appears — not a security
          release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The "Scoring" link, which opens an explanation of how documents are graded, moved from the
        top of the page to the footer alongside the other reference links. The explanation itself is
        unchanged.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        <h3 class="text-lg font-bold text-[var(--text-heading)]">v1.40.1</h3>
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: one remaining third-party update —
          <strong>a security release</strong>.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        Applies the last outstanding fix left over from the previous release, in a tool used only
        while developing the application — never on the live server. One advisory remains open and
        cannot be acted on yet: it concerns a form component in the interface library this site
        uses, and its authors have not published a fix. It does not affect this application, which
        does not use the component in question. It will be applied as soon as a fix exists.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        ...
        maintained by others. When a flaw is found in one of those components, a security advisory
        is published and the component's authors release a fix. This release applies every
        outstanding fix — around 25 advisories in total, several rated high severity.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        Every one of these was in a supporting component used to build and package the application
        rather than in the code that reads your documents, and we have no indication any was ever
        exploited here. They are applied because keeping dependencies current is basic maintenance,
        not because a problem was observed.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        One component that <em>is</em> used to read your documents was updated: the XML reader
        behind Word, PowerPoint and Excel checks. Because a subtle change there could shift scores
        without producing any visible error, four sample documents were audited on both the old and
        new versions and the results compared — they were identical. The status page also gained a
        Chicago-local timestamp alongside the existing UTC one.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The counts on the public status page are now recalculated every few seconds rather than once
        a minute, so a document that has just been audited appears in the totals almost immediately.
        This changes only how current those totals are. It does not change what is counted, and the
        page still publishes nothing beyond totals — no file names, no email addresses, and nothing
        identifying anyone who has used the service.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        announcements introduced in the previous release. That archive was previously reachable only
        from the notice banner on the home page, which can be dismissed permanently — so the list of
        past updates vanished exactly when someone would want it. These are ordinary navigation
        links to a page showing notices already published on the home page.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
      </p>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-08-03</strong> · scope: a broken link and a new archive page — not a
          security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The link to the new status page, added in the previous release, produced a "page not found"
        error when clicked from the home-page banner, though the page itself worked normally when
        its address was typed directly. That is fixed. This release also adds a page listing every
        past announcement, since the banner shows only the most recent one and can be dismissed.
        That archive displays the same notices already shown on the home page and reads nothing from
        your documents.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        ...
        As first shipped it published only <strong>totals and yes/no answers</strong>: whether the
        service is running, whether each checking engine is working, how long the server has been
        up, and how many documents have been audited in the last day, the last month, and in total —
        split by file type. Later releases added further anonymous totals: letter-grade counts
        (v1.44.0) and refused-upload counts (v1.46.0) — see those entries. Then as now, the page
        publishes no file name, email address, IP address, browser identifier, or document
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        Two figures were considered and deliberately left out.
        <strong>Report-sharing counts</strong> were rejected because sharing cannot actually be
        observed: the system records that a report was created, never whether its link was sent to
        anyone, so any number published under the word "shared" would claim more than the software
        can know. <strong>Web-page audit counts</strong> were left out because the distinction
        between a document and a web page raises more questions than it answers for a general
        reader. Two further points of note: the page is excluded from search engines by two
        independent mechanisms, and when a checking engine fails, the page reports a fixed short
        label rather than the underlying error message, because those messages routinely contain
        server file paths.
        <strong
          >No change to what data is collected, how it is used, or how long it is kept.</strong
        >
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The previous release moved one technical panel below the list of issues that must be fixed,
        but there are two, and the other one was missed: a "PDF/UA-1 signals" card that appeared at
        the very top of the report, immediately under the score and above the critical issues. Both
        now sit below the issues. The distinction matters because these signals are easy to mistake
        for a passing grade: they report structural facts about the file — whether it carries
        accessibility tags, whether its fonts are embedded — and cannot judge whether an image
        description is meaningful or whether the document reads in a sensible order, which is what
        the accessibility grade measures. A document can therefore satisfy every one of these
        structural markers and still be unusable for someone with a disability. When critical issues
        remain, the card now says that plainly instead of leaving the reader to infer it. This is a
        presentation change only — no scores, grades, or verdicts changed, and no new information is
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        A report can carry two different verdicts that mean different things, and they were shown in
        a misleading order. The technical PDF/UA-1 check (which examines whether a PDF's tagging is
        formally well-formed) was displayed above the list of accessibility issues that actually
        ...
        green tick. This is a presentation change only — no scores, grades, or verdicts changed, and
        no new information is read from your documents.
        <strong>No change to what data is collected or how long it is kept.</strong>
      </p>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        This review looked at the audit engine with fresh eyes and found three ways a document could
        be judged wrongly — all of them in the direction of being too generous, which is the more
        damaging direction for a compliance tool. The most serious: a PDF could carry an
        ...
        identical file with the empty shelf removed was correctly failed. In other words, a document
        could be made to "pass" by adding tagging that did nothing. Empty tagging is now treated
        exactly like no tagging, because that is what it means for someone using a screen reader.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        uploads could exhaust the server's memory. It is now limited to two at a time. Finally, when
        that checker failed it was reporting the server's own internal file paths back to the
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Reviewed <strong>2026-07-22</strong> · scope: extends the v1.36.2 PDF image fix to lists
          and tables, from the same reported document — not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The same "phantom tag" problem fixed for images in v1.36.2 also affected lists and tables: a
        design tool had left behind dozens of empty list and table tags that are not part of the
        document a screen reader reads, and the audit was reporting them as broken ("incomplete")
        structure and lowering the score. The tool now ignores these disconnected tags for lists and
        tables as well, so the reported document is scored on its real content only. This changes
        only how existing information in the file is interpreted — no new information is read from
        your documents.
        <strong>No change to what data is collected or how long it is kept.</strong>
      </p>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        now ignores image tags that are not connected to the live document structure, and correctly
        recognizes when every image on a page is deliberately marked as decorative and therefore
        needs no description. This changes only how existing information in the file is interpreted
        — no new information is read from your documents.
        <strong>No change to what data is collected or how long it is kept.</strong>
      </p>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        v1.36.0 makes the audit's judgments more trustworthy in both directions. The review found
        places where the tool accused documents of accessibility failures they did not have — for
        example, white text on dark table headers (a correct, accessible design) was being reported
        as an extreme color-contrast violation because the tool didn't look up the header's
        background color, and slide decks that record their language on every line of text were told
        they had "no language declared". Those false alarms are fixed: the tool now only asserts a
        confirmed WCAG failure when it actually resolved the evidence from the file, and says "not
        assessed — review manually" when it could not.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        The review also closed the reverse problem — a serious barrier that used to pass silently: a
        PDF whose (older-style) security settings forbid screen readers from reading it at all could
        previously receive a perfect score. Such files are now failed with a clear explanation and
        fix. The tool additionally reads more of each document than before (Word headers, footers
        and footnotes, legacy link and image formats, Excel chart sheets), so fewer barriers can
        hide in unread corners.
        <strong>No change to what data is collected or how long it is kept.</strong>
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-07-19</strong> · scope: a new automated status-check address used for
          uptime monitoring — not a security release.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        v1.35.0 adds a single public status-check address that reports whether both halves of the
        service — the website itself and the analysis engine behind it — are running, so an external
        monitoring service can alert the team the moment either goes down. The check was reviewed
        before shipping: it reveals only "running or not" for each half and how long the analysis
        engine has been up — no user data, no file names, and nothing about any audit anyone has
        run. It was also designed so that deliberately overloading the status check cannot trick the
        monitoring service into reporting a false outage.
        <strong>No change to what data is collected or how long it is kept.</strong>
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-07-12</strong> · scope: five preventive hardening measures covering
          file uploads, sign-out, and auto-remediation status pages, alongside an internal code
          reorganization and a new automated test/quality pipeline.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        This release adds five defensive improvements identified during a routine internal review of
        the whole application.
        <strong
          >None of them close a hole that was ever actually used against the tool — think of it as
          adding a second lock to a door that already had one, not replacing a broken lock.</strong
        >
        The same review also reorganized how the audit engine's code is packaged internally (no
        change to what it checks, how it scores, or what data it collects) and added an automated
        pipeline that runs the full test suite, a code-style check, and a type-correctness check on
        every change pushed to the repository — so future changes are checked automatically going
        forward, not only when someone remembers to run the tests by hand.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          opens any of those pieces, how many there are and how large they would add up to be once
          uncompressed, and refuses a bundle that crosses a safe ceiling. This closes a gap the
          per-piece checks already in place (added when Word, PowerPoint, and Excel auditing first
          shipped) didn't cover on their own: a bundle made of an extreme number of small pieces, or
          one whose pieces add up to an extreme total.
        </li>
        <li>
          <strong
            ><span
          ...
          >
          — Word, PowerPoint, and Excel files are built internally from a markup language that has a
          handful of advanced features no ordinary document ever needs, but that a booby-trapped
          file could misuse to make the reading process balloon in memory or reach outside the file.
          The tool now recognizes this specific feature on sight and treats that piece of the file
          as empty rather than processing it. Genuine Word, PowerPoint, and Excel exports never use
          it, so no ordinary file is affected.
        </li>
        <li>
          <strong
            ><span
          ...
          >
          — Previously, clicking sign-out cleared your browser's copy of your sign-in credential,
          but if a copy of that credential had ever been captured some other way, it would
          technically have remained usable until it expired on its own. The server now keeps a short
          record of every sign-out and immediately rejects that exact credential if it is ever
          presented again, so sign-out is final the moment you click it. (Sessions that began before
          this change shipped aren't covered by this new check, but they still expire on their own
          normal schedule, same as always.)
        </li>
        <li>
          <strong
            ><span
          ...
          >
          — When you start an auto-remediation job without signing in, checking that job's progress
          or its completion receipt now requires the same private, single-use address you were given
          when the job started. Anyone without it is told the job doesn't exist, rather than being
          able to check on it by guessing or reusing an identifier.
        </li>
        <li>
          <strong
            ><span
          ...
          >
          — Every update to the tool that changes the internal database's structure is now numbered
          and recorded, so the server always knows exactly which structural updates a given
          installation has already received and applies only the ones it's missing, in order,
          automatically — including on the existing production database. This replaces a less formal
          check-before-change approach and removes a way a future update could have been skipped or
          mistakenly reapplied.
        </li>
      </ul>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong>No change to what data is collected or how long it is kept.</strong>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-07-03</strong> · scope: the new PowerPoint (.pptx) and Excel (.xlsx)
          audit features — a fresh, independent three-team red/blue review of everything a malicious
          Office file could try to do to the server.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        v1.33.0 extends the tool to audit PowerPoint and Excel files, not just PDF and Word. Because
        these are also user-supplied files the server has to open and parse, this release got the
        same treatment as the earlier Word rollout: three independent reviews — covering server
        overload, hidden malicious content, and ways the scoring or access rules could be tricked —
        deliberately tried to break it with poisoned, oversized, and malformed files.
        <strong
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          <strong
            ...
            force</strong
          >
          — A PowerPoint or Excel file can bury thousands of objects several layers deep, or pair a
          small file with a few oversized embedded pictures, to make the server do far more work
          than the file's size suggests. The tool now counts that work — shapes, text, and cells at
          every nesting depth, and the running byte size of embedded pictures — and stops as soon as
          a safe limit is crossed, instead of after the damage is already done.
        </li>
        <li>
          <strong
            ><span
          ...
          in their own short-lived process that the server can immediately and completely cancel the
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        browser to <strong>refuse any script that wasn't part of the original page</strong>, so this
        whole category of attack is blocked even if a new bug were introduced later.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          >
          — A shareable report link could be hand-crafted so that a link on it, once clicked, ran a
          hidden script instead of opening a web page. Link addresses on saved reports are now
          verified to be ordinary web (http/https) addresses both when the report is saved and when
          it is displayed, so a disguised script address is dropped.
        </li>
        <li>
          <strong
            ><span
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong>No change to what data is collected or how long it is kept.</strong>
        These are display-and-safety changes only; uploaded files are still processed in memory and
        discarded in seconds. The full technical write-up is in the project's README security
        section.
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-07-01</strong> · scope: the new Microsoft Word (.docx) audit feature
          — a fresh, independent red/blue team review of everything a malicious Word file could try
          to do to the server.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        >, because the tool escapes every piece of text taken from an uploaded document before it is
        ever displayed. Everything the review found was a way to overload the server, and
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        ...
        <li>
          ...
          >
          — Word audits run through the same "two at a time" queue and the same hard time limit that
          PDF audits already used, so no single upload (or flood of uploads) can starve the server
          of resources.
        </li>
        <li>
          <strong
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong>No change to what data is collected or how long it is kept.</strong>
        Word files are processed in memory and discarded in seconds, exactly like PDFs; nothing new
        is stored or transmitted. The full technical write-up is in the project's README security
        section.
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          ...
          requests, and an optional access key for a trusted partner system. Reviewed for security;
          no change to what data is collected or how long it is kept.
        </span>
      </header>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
        ...
        public tool can't be hammered with thousands of automated requests an hour. A trusted ICJIA
        system can present a secret access key to get a higher limit and to check ICJIA pages that
        live on non-Illinois web addresses.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        <strong>No data is collected, stored, transmitted, or retained any differently</strong>; no
        retention window changed. The access key only raises rate limits and widens which web
        addresses can be checked — it never lets anyone reach internal or private systems (those
        stay blocked for everyone), and it is held only as a server environment secret, never in the
        database or in any report.
      </p>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          <strong>2026-06-10</strong> · scope: a small fix to make a loading spinner icon appear. No
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        screen.
        <strong>No data is collected, stored, transmitted, or retained any differently</strong>; no
        retention window, endpoint, or permission changed.
      </p>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        <strong>No data is collected, stored, transmitted, or retained any differently</strong>; no
        retention window, endpoint, or permission changed. Your audited files are still held in
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-06-10</strong> · scope: a full, independent red-team security review
          of the entire application — the website, the server, the audit pipeline, and the optional
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        found
        <strong>no critical issue and no way for one user to reach another user's data</strong>: the
        high-impact vulnerability classes (database injection, command injection, file-path escape,
        cross-site scripting, and login bypass) were each tested and verified clean. The items found
        were hardening against denial-of-service and against future misconfiguration, and
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        ...
        <li>
          ...
          login secret is ever misconfigured, removal of the sharer's email from public share links,
          and several smaller defensive fixes. No code path that stores, transmits, or retains your
          data changed; no retention window, endpoint, or permission changed.
        </li>
      </ul>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        (previously these failed immediately, even though they are exactly the files remediation is
        for), flags machine-generated download filenames used as document titles so they are not
        mistaken for real titles, and restores the loading spinner icon. No security-relevant
        behavior changed.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          >
          — A document with a small file defect is repaired during intake instead of being rejected,
          matching how the audit itself reads such files since v1.26.0.
        </li>
        <li>
          <strong
            ><span
          ...
          with a note to write a real title; short legitimate titles like "COVID-19" are unaffected.
          Some documents' Title &amp; Language score may move slightly.
        </li>
        <li>
          <strong
            ><span
          ...
          >
          — The remediation change reads a status code and a file the tool already wrote inside the
          job's own working folder; the icon fix bundles an image set at build time. No endpoint,
          retention window, or permission changed.
        </li>
      </ul>
    </article>
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-06-10</strong> · scope: accuracy fixes across the PDF analysis engine
          — how the document file is read, how tables, forms, lists, and titles are judged, and when
          the report may claim a confirmed WCAG failure. No security review was required — nothing
          about data handling changed.
        </span>
      </header>

      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        v1.26.0 corrects cases where the audit reported things that were not true about a document.
        The most important: a PDF with a minor, repairable file defect (common in older or re-saved
        documents) was scored as if it had no accessibility tagging at all — the identical document
        could score 100 or 42 depending on that one defect. The release also stops several false
        alarms, closes a detection gap, and re-verifies every "How to fix" instruction against
        Adobe's current documentation. An independent code review was completed before release.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          reported as untagged. Some previously low scores on tagged documents will rise to reflect
        ...
        <li>
          ...
          label are no longer failed; and a table nested inside another is no longer counted twice.
        </li>
        <li>
          <strong
            ><span
          ...
          measured the tag order against the visual order, and only claims a missing title when the
          document truly has none.
        </li>
        <li>
          <strong
            ><span
          ...
          code path that stores, transmits, or retains data changed; no endpoint, retention window,
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          ...
          link and reading-order scoring calibration, and a new PDF/UA-1 conformance-signals panel.
    ...
    >
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          >
          — The audit labels, conformance verdict, exports, and UI copy all reference WCAG 2.2 AA (a
          strict superset of WCAG 2.1 AA). New 2.2 criteria are shown as "not assessed — manual
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        <strong>WCAG conformance verdict</strong> is <em>displayed</em>. It does not change what the
        audit checks, what data is collected, where it is stored, or how long it is kept. No new
        endpoints, no authentication change, no retention change.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          a single gap is still worth fixing, but a strong grade still means the document is in good
          shape. The box is shown in green for strong documents and red for weak ones; every flagged
          issue is still listed in full, whatever the color.
        </li>
        <li>
          <strong
            ><span
        ...
        <li>
          ...
          audit already produced, the downloadable reports are unchanged, and no new information is
          sent or stored anywhere. Every defensive control from prior releases remains in force.
        </li>
      </ul>
    </article>
    ...
    >
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        ...
        <li>
          ...
          >
          — Category weights and some labels were recalibrated to match WCAG conformance levels more
          honestly. As a result, a score produced by v1.22.0 is not directly comparable to a score
          from an earlier version. An audit campaign that spans this upgrade will see numbers move;
          that movement reflects the improved methodology, not a change in the documents.
        </li>
        <li>
          <strong
            ><span
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            shared-report template. The underlying <em>data</em> is still saved in the database (so
            historic API consumers that already parse it keep working), but it's no longer rendered
            on the page. No schema change. The per-finding "How to Fix in Adobe Acrobat" remediation
            guidance inside each category card is kept — that's per-finding remediation advice, not
            a separate scoring profile, and it appears on the live audit page too.
          </p>
        </li>
        <li>
          <strong
          ...
          pass. The elevated limit will stay in place for the duration of the campaign and revert to
          a tighter number once it concludes.
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            <strong>Why this is OK:</strong> the per-caller analyze limit is a fair-use throttle.
            The actual abuse mitigations live on the remediation side — the 100/day remediation cap
            per caller, the 60-minute audit-gate
        ...
        <li>
          ...
          migration. No change to the authentication layer, the SSRF allowlist, the audit-gate hash
          check, the daily remediation cap, the retention windows, or the URL-fetch posture.
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            The two changes in this release are a 5-line UI deletion on the shared-report template
            and a single numeric raise on one rate-limit constant. No other code paths were touched.
          </p>
        </li>
      </ul>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        two scoring profiles at once — "Strict" and "Practical" — and asked users to choose between
        them. That cognitive load got in the way of the actual accessibility findings. After review,
        the team retired Practical and kept Strict, which is the WCAG 2.1 AA + IITAA
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            <strong>What was wrong:</strong> showing two profiles created an implicit "which one is
            correct?" question for the reader. The Strict view is what Illinois IITAA and the ADA
            point to; the Practical view layered a separate PDF/UA-flavored weighting on top, which
            was useful for tool reconciliation but not for publication decisions.
            <br />
            <strong>What this release does:</strong> the audit page now shows only the Strict /
            WCAG-anchored score. The underlying scoring engine is unchanged — same nine categories,
            same weights, same WCAG-anchored thresholds. Just less noise on the page.
          </p>
        </li>
        <li>
          <strong
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            below the score; the detailed section below was renamed to "PDF/UA-1 conformance check"
            so its purpose is obvious; the badge jumps to that section for the full rule failure
            list when failures exist. When veraPDF isn't installed on the server, the badge clearly
            reads <em>"check not run"</em> rather than pretending the check ran successfully.
          </p>
        </li>
        <li>
          <strong
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            updates. The alias will be removed in a future release once we've confirmed no consumer
            depends on the values differing.
          </p>
        </li>
        <li>
          <strong
            ...
            No security regressions</strong
          >
          — All SSRF, rate-limit, audit-gate, daily-cap, and retention controls from v1.20.1 remain
          in force. The cleanup pass still purges remediation files, jobs, and audit-log rows on
          schedule.
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            The simplification is a UI and scoring-presentation change. It does not modify the
            upload pipeline, the authentication layer, the rate limiters, the audit-gate hash check,
            the daily cap, the SSRF protections, or the retention windows.
          </p>
        </li>
      </ul>
    </article>
    ...
    >
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            domain — for example, by compromising a partner agency that operates a subdomain — they
            could point that hostname at the server's loopback address (127.0.0.1) and trick us into
            ...
            link-local, or multicast ranges. The check repeats on every redirect hop so a redirector
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            fetches each one. It had its own private fetcher with no allowlist — an authorized user
            could submit a list containing internal addresses and the tool would fetch them. Latent
            since the endpoint shipped, not previously discovered.<br />
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            policy. An attacker repeatedly auditing unique files could slowly fill the database.<br />
        ...
        <li>
          ...
          limit.
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            <strong>What was wrong:</strong> the daily-limit check and the actual job-creation were
            two separate steps. Two perfectly-simultaneous requests at the cap boundary could both
            see "you're under the limit" and both proceed.<br />
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            flags (HttpOnly, Secure, SameSite-Strict) that prevent it from being read by client-side
            scripts, transmitted over plain HTTP, or sent with cross-site requests.<br />
      ...
      <ul class="space-y-1 text-sm text-[var(--text-secondary)] list-disc list-inside ml-2">
        <li>
          <strong>Audit required before remediation.</strong> Every request to remediate a PDF must
          be preceded by an audit of the same content within the previous 60 minutes. Any audit path
          counts — direct upload, URL audit, or fleet bulk. This prevents automated abuse where
          someone bypasses the audit pipeline and floods the remediation worker directly.
        </li>
        <li>
        ...
        <li>
          ...
          paths. The hash is just a fingerprint — it doesn't expose the PDF's contents and can't be
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-2">
        ...
        <strong>every feature ships through a fresh red/blue team review before tagging</strong>.
        The review examines the newly-introduced surface from a sophisticated-adversary perspective,
        looks for attack patterns like DNS rebinding, race conditions, identity collapse, and
        slow-burn denial-of-service, and either fixes findings in the same release window or
        documents them for future work. This release (v1.20.1) is the security-followup to v1.20.0,
        which added the fleet-audit-by-URL feature. The pattern repeats with every feature release —
        earlier entries in this audit history list the findings from prior reviews.
      </p>
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        For a manager reading this page: the intent here is transparency. The tool is built and
        reviewed iteratively, and this page is the auditor-readable trail of what was reviewed, what
        was found, what was fixed, and what was deliberately accepted with mitigation. The technical
        equivalent (with full code references) lives in
    ...
    >
      ...
      <h4 class="text-sm font-semibold text-[var(--text-heading)] mb-2">Findings &amp; changes</h4>
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          <strong
            ><span
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            file is now saved under the same filename they uploaded — including any spaces, unicode,
            or punctuation. The download dialog presents three options with "Keep original filename"
            pre-selected and badged Recommended. The other two ("Add a _remediated suffix" or "Use a
            different filename") are opt-in.<br />
            ...
            emails, in shared documents, keeps working as long as the filename matches. The previous
            behavior automatically appended <em>_remediated</em> to the filename, which broke this
            workflow.<br />
            ...
            treated only as a display name for the browser's save dialog and is capped, encoded, and
            forced to <code class="text-xs font-mono">.pdf</code>
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            gained a "PDF (browser print)" button. Clicking it opens the browser's own print dialog,
            where the user picks "Save as PDF" as the destination. The page applies a print
            stylesheet that hides interactive controls, switches to black-on-white text, expands
            collapsed technical sections, and arranges page breaks cleanly.<br />
            ...
            feature. The chosen filename is whatever the user types in the browser's save dialog and
      ...
      <ul class="space-y-1 text-sm text-[var(--text-secondary)] list-disc list-inside ml-2">
        <li>
          New <code class="text-xs font-mono">AGENTS.md</code> at the repository root documents the
          load-bearing conventions for AI coding agents (Claude Code, Codex, Cursor, etc.) so
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        practical grades plus a year-long shareable report link), expands the URL allowlist to cover
        all
        <code class="text-xs font-mono">*.illinois.gov</code>
        state-agency subdomains, bumps the shared-report retention window from 15 days to 365 days,
        and fixes seven accessibility rule violations across the public policy + technical-details
        pages. The most material policy change for an auditor reading this page is the retention
        bump — see the first finding below.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            audit-by-URL automation), the resulting link now stays valid for one year instead of 15
            days. This applies to the metadata record only — no PDF content is stored alongside it.
            After 365 days the row becomes eligible for the periodic cleanup sweep and the URL stops
            working.<br />
            ...
            no PDF bytes. A 100-PDF fleet at roughly 50 KB per record grows the database by about 5
            MB per year. The tradeoff was evaluated and accepted in favor of usability.
          </p>
        </li>
        <li>
          <strong
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            networks), still rejects oversized files (100 MB cap), still requires the fetched bytes
            to begin with the <code class="text-xs font-mono">%PDF-</code> header, and still rejects
            look-alike domains (a URL like
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            boxes on these pages couldn't be reached via keyboard, that an inline link in this audit
            history section was distinguishable only by color (a barrier for colorblind readers),
            and that several scrollable code blocks couldn't be scrolled without a mouse.<br />
            <strong>How it was fixed:</strong> each scrollable region is now keyboard-focusable, the
            inline link is now underlined, and the diagram boxes' redundant ARIA labels were
            replaced with proper structural markup. Both pages now score a perfect 100 / 100 on both
            axe (no violations) and Lighthouse's accessibility audit.
          </p>
        </li>
        <li>
          <strong
            ...
            Fixed</strong
          >
          — The new fleet endpoint reported the strict score in both the strict and practical slots
          of its response.
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            practical one.<br />
            <strong>How it was fixed:</strong> caught in the local smoke-test step before any caller
            integrated against the endpoint, so no production fleet report ever published the wrong
            number. The name mapping is now correct (verified against three test PDFs whose strict
    ...
    >
      <header class="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
        ...
        <span class="text-xs text-[var(--text-muted)]">
          Audited <strong>2026-05-18</strong> · scope: veraPDF integration correctness + remediation
          result-page UX
        </span>
      </header>

      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        A patch release with four operational fixes against the v1.18.0 remediation feature. None of
        these findings expose private data or change the file-retention guarantees described
        ...
        this feature flag was still off in production, so no real audit was shown the wrong verdict.
      </p>
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            veraPDF also moved its rule-by-rule detail list. A defensive fallback in the tool would
            have tried to read the new "count of failed rules" as if it were a list, which would
            have crashed the validation routine on certain inputs.<br />
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            <strong>How it was fixed:</strong> the tool now uses veraPDF's own total when available.
            Older veraPDF versions still use the "sum the displayed list" fallback.
          </p>
        </li>
        <li>
          <strong
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            <strong>How it was fixed:</strong> each issue row now opens an inline accordion showing
            the detailed findings and numbered Adobe Acrobat fix steps right there on the result
            page — no navigation needed. Same content as the audit-page cards. Not a privacy or
            security issue, but a real usability problem for an auditor following up on outstanding
            items.
          </p>
        </li>
      </ul>
      ...
      <ul class="space-y-1 text-sm text-[var(--text-secondary)] list-disc list-inside ml-2">
        <li>
          ...
          auto-detects an installed veraPDF and, when it isn't installed, prints copy-paste install
          instructions including the persistence command so the path survives a server reboot.
          Reduces drift between development and production installs.
        </li>
      </ul>
    </article>
    ...
    >
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
        ...
        red/blue team review covered the public API endpoints, the worker, the frontend, the cleanup
        sweep, the database schema, and the file lifecycle. The 15-row threat-model checklist
        documented in
      ...
      <ul class="space-y-3 text-sm text-[var(--text-secondary)] mb-4">
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            <strong>What was wrong:</strong> the download endpoint loaded the entire remediated PDF
            (up to 50 MB) into the API process's memory before sending it to the user's browser.
            Under several simultaneous downloads, this could exceed the API process's 512 MB memory
            cap and crash it. <br />
            <strong>How it was fixed:</strong> switched to streaming the file in small chunks (<code
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            two near-simultaneous requests with the same token could both pass the validation check
            and both retrieve the file before either completed. This violated the "single-use"
            privacy guarantee.<br />
            <strong>How it was fixed:</strong> the job is marked
            <code class="text-xs font-mono">status='expired'</code>
            <em>before</em> the file is sent, so any concurrent second request immediately sees the
            expired status and receives a "410 Gone" response.
          </p>
        </li>
        <li>
          <strong
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            <strong>What was found:</strong> when the tool runs with the "require login" flag turned
            off (typical for internal development), the per-job email check on the status, download,
            and receipt endpoints is bypassed. Anyone who knows a job's UUID could read its data.<br />
            <strong>How it was handled:</strong> job UUIDs use 122 bits of cryptographic randomness
            — guessing one is computationally impractical. Production deployments run with login
            required, which closes the gap entirely. This limitation is documented in the
            integration plan as the known posture; it does not affect the production deployment.
          </p>
        </li>
        <li>
          <strong
        ...
        <li>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            ...
            serve a file that <em>might</em> be damaged and contaminate the user's records. Privacy
            and integrity over feature completion.
          </p>
        </li>
      </ul>
      ...
      <ul class="space-y-1 text-sm text-[var(--text-secondary)] list-disc list-inside ml-2">
        ...
        <li>
          Full automated test coverage for the remediation pipeline (<code class="text-xs font-mono"
    ...
    <article class="rounded-xl border border-[var(--border)] bg-[var(--surface-card)] p-5 sm:p-6">
      ...
      <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
        Security reviews for releases prior to v1.18.0 were not yet captured in this format. Earlier
        releases focused on the synchronous audit pipeline (added in v1.0) and authentication flow
        (Personal Access Tokens added in v1.16, analyze-by-URL added in v1.17). Review history for
        those releases is available via the
        <a
        ...
        >. Going forward — beginning with v1.18.0 — every release will have a corresponding entry in
        this section before tagging.
      </p>
    </article>
  </section>
</template>
