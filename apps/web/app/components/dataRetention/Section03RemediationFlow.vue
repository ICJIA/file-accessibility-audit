  ...
  <section id="remediation-flow" class="scroll-mt-8">
    ...
    <ul class="text-sm text-[var(--text-secondary)] list-disc list-inside ml-2 space-y-1.5">
      <li>
        ...
        <strong>at most one copy of the PDF exists on disk</strong>. The input is deleted before the
        normalized intermediate is written for downstream stages; the normalized intermediate is
        deleted before the tagged output is finalized; the tagged output is deleted on first
      ...
      <li>
        ...
        between audit and remediation. Clicking "Remediate" triggers a fresh multipart upload — the
        just-audited buffer is not preserved. This is a deliberate UX-vs-privacy trade-off that
        costs the user one extra upload click in exchange for a stricter retention posture.
      </li>
      <li>
