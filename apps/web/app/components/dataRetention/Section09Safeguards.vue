<template>
  ...
  <section id="safeguards" class="scroll-mt-8">
    ...
    <ul class="space-y-2 text-sm text-[var(--text-secondary)] list-disc list-inside ml-2">
      <li>
        <strong>HTTPS / TLS 1.2+</strong> on all transport between client and server. The production
      ...
      <li>
        ...
        Excel upload must be a well-formed ZIP (OOXML) package whose internal parts confirm which of
      ...
      <li>
        <strong>Page count cap</strong>: 500 pages for remediation (configurable). Pathological PDFs
        with thousands of pages are rejected before any processing.
      </li>
      <li>
      ...
      <li>
        <strong>Wall-clock timeout</strong> on the remediation worker: 5 minutes (configurable). The
      ...
      <li>
        ...
        database — jobs stuck in "running" for over 10 minutes are marked failed; orphan files with
        no matching database row are removed.
      </li>
      <li>
        <strong>Regression guard on remediation</strong>: the output PDF is rejected if its overall
        or Strict (WCAG + IITAA §E205.4) score regresses relative to the input. The user never sees
        an output that would make any visible metric worse.
      </li>
    </ul>
  </section>
</template>
