<template>
  ...
  <section id="tools" class="scroll-mt-8">
    <h2 class="text-2xl font-bold text-[var(--text-heading)] mb-3">5. The open-source toolchain</h2>
    <p class="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">
      Every tool involved in processing an uploaded document — PDF, Word, PowerPoint, or Excel — is
      open source, license-clear, and runs locally on the ICJIA-controlled server. No commercial PDF
      or Office-document SDK is licensed, and no per-document fees are paid. The tools are:
    </p>
      ...
      <table class="w-full text-sm">
        ...
        <thead>
          <tr class="text-left text-[var(--text-muted)] border-b border-[var(--border)]">
            ...
            <th scope="col" class="py-2 pr-4 font-medium">Used for</th>
        ...
        <tbody class="text-[var(--text-secondary)]">
          <tr class="border-b border-[var(--border)]/40">
            ...
            <td class="py-3 pr-4 text-xs">
              PDF structure parsing (audit) and PDF normalization + validity checking (remediation)
            </td>
    ...
    <p class="text-sm text-[var(--text-secondary)] mt-4 leading-relaxed">
      ...
      running inside a dedicated, short-lived child process spawned for that one request (JSZip and
