<template>
  <div>
    ...
    >
      ...
      <table class="w-full text-sm min-w-[420px]">
          ...
          >
            ...
            <td class="text-center px-2 sm:px-3 py-2.5">
              <span
                ...
                >{{ cat.severity }}</span
              >
              <span v-else class="text-[var(--text-muted)] text-xs" aria-hidden="true">—</span>
            </td>
          </tr>
        </tbody>
        <tbody v-if="naCategories.length">
          ...
          >
            ...
            <td class="text-center px-3 py-2.5 font-mono text-[var(--text-muted)]">
              <NaCell :cat-id="cat.id" :not-assessed="cat.notAssessed" />
            </td>
            <td class="text-center px-3 py-2.5 text-[var(--text-muted)]" aria-hidden="true">—</td>
            <td class="text-center px-3 py-2.5 text-[var(--text-muted)] text-xs" aria-hidden="true">
              —
            </td>
          </tr>
        </tbody>
      ...
      >
        <p class="font-medium text-[var(--text-secondary)]">About the N/A rows</p>
        <p>
          <strong>N/A</strong> means this analyzer abstained from a score for that category under
          the active mode — it does <em>not</em> mean the category is exempt from WCAG, ADA, or
          IITAA.
          <strong
            >Hover or keyboard-focus the
            <span
            ...
            ></strong
          >
          on any N/A cell to read the specific reason (Strict doesn't include a PDF/UA category;
          Reading Order requires page-stream analysis this tool hasn't implemented; some categories
          don't apply to every file format; small documents don't require bookmarks; etc.).
        </p>
      </div>
    </div>
    ...
    >
      ...
      <div class="divide-y divide-[var(--border-subtle)]">
        ...
        >
          <span class="sm:w-40 sm:flex-shrink-0 text-[var(--text-muted)] text-xs sm:text-sm">{{
            item.label
          }}</span>
          <span
      ...
      >
        ...
        >
          ...
          <ol class="px-4 py-3 space-y-2">
            <li
              v-for="(step, j) in partitionCardFindings(cat.findings).acrobat"
              :key="j"
              class="text-sm text-[var(--text-muted)] flex gap-2.5"
            >
              <span class="flex-shrink-0 text-amber-400/70 font-mono text-xs mt-0.5 w-4 text-right"
                >{{ j + 1 }}.</span
              >
              <span>{{ step }}</span>
            </li>
        ...
        >
          ...
          <ul class="divide-y divide-indigo-500/15">
            <li v-for="c in getWcagCriteria(cat.id)" :key="c.id + c.name">
              ...
              >
                <span class="font-mono text-sm text-indigo-300 flex-shrink-0 w-12">{{ c.id }}</span>
                <span
        ...
        <div v-if="cat.helpLinks?.length" class="mt-3 pt-3 border-t border-[var(--border-subtle)]">
          <span class="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide"
            >Learn more</span
          >
    ...
    <div v-if="naCategories.length">
      <h2 class="text-base sm:text-lg font-semibold mb-4 mt-8 text-[var(--text-secondary)]">
        Not Included in Scoring
      </h2>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <ul class="space-y-1.5 max-h-96 overflow-y-auto">
            ...
            >
              <span class="flex-shrink-0 mt-0.5 font-bold" :style="{ color: 'var(--icon-na)' }"
                >–</span
              >
              <span>{{ finding }}</span>
            </li>
...
const displayedCategories = computed(() =>
  categoriesForScoringMode(safeCategories.value, props.result.scoreProfiles, "strict"),
);

const scoredCategories = computed(() => displayedCategories.value.filter((c) => c.score !== null));
const naCategories = computed(() => displayedCategories.value.filter((c) => c.score === null));
const hasAnyNaRow = computed(
  () => naCategories.value.length > 0 || scoredCategories.value.some((c) => c.score === null),
);
