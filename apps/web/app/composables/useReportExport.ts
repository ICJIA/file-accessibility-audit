...
function buildMarkdown(result: ReportResult): string {
  const lines: string[] = []
  ...
  lines.push('---')
  lines.push('')
  lines.push(`*Report generated by [ICJIA File Accessibility Audit](https://audit.icjia.app) — ${timestamp()}*`)
...
function buildJSON(result: ReportResult): string {
  ...
  const report = {
    reportMeta: {
      ...
      tool: 'ICJIA File Accessibility Audit',
      toolUrl: 'https://audit.icjia.app',
      schemaVersion: '2.0',
    },
    file: {
...
async function buildDocx(result: ReportResult): Promise<Blob> {
  ...
  detailParagraphs.push(
    new Paragraph({
      children: [
        ...
        new ExternalHyperlink({
          children: [new TextRun({ text: 'ICJIA File Accessibility Audit', size: 18, font: 'Calibri', italics: true, color: '3b82f6', underline: {} })],
          link: 'https://audit.icjia.app',
        }),
...
<div class="container">
  ...
  <div style="margin-top:40px;padding-top:16px;border-top:1px solid #222;text-align:center">
    <p style="font-size:12px;color:#555">Report generated by <a href="https://audit.icjia.app" target="_blank" rel="noopener noreferrer" style="color:#60a5fa">ICJIA File Accessibility Audit</a> — ${timestamp()}</p>
  </div>
...
export function useReportExport() {
  ...
  async function exportDocx(result: ReportResult) {
    ...
    try {
      const blob = await buildDocx(result)
