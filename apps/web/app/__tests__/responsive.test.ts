...
describe('Responsive — CategoryRow', () => {
  ...
  it('has a desktop-only chevron (hidden sm:block)', () => {
    const wrapper = mount(CategoryRow, { props: { category: baseCategory } })
    const svgs = wrapper.findAll('svg')
    const desktopChevron = svgs.find(s => s.classes().includes('sm:block') && s.classes().includes('hidden'))
...
describe('Responsive — Report Page', () => {
  ...
  it('grade circle uses responsive sizing (w-24 sm:w-32)', () => {
    expect(source).toContain('w-24')
    expect(source).toContain('sm:w-32')
  })

  it('grade letter uses responsive font size (text-4xl sm:text-6xl)', () => {
    expect(source).toContain('text-4xl sm:text-6xl')
  })
