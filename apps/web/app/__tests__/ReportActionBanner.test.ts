...
describe("ReportActionBanner", () => {
  ...
  it("critical + moderate combined", () => {
    ...
    expect(wrapper.text()).toBe(
      "2 critical and 1 moderate issues must be fixed before publishing.",
    );
  });
  ...
  it("minor only, singular", () => {
    ...
    expect(wrapper.text()).toBe(
      "1 minor issue found. Optional fixes — this document passes Illinois accessibility.",
    );
  ...
  it("minor only, plural", () => {
    ...
    expect(wrapper.text()).toBe(
      "2 minor issues found. Optional fixes — this document passes Illinois accessibility.",
    );
  });
