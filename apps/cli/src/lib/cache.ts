...
export interface CachedRow {
  ...
  pdf_ua_compliance_score: number | null;
  pdf_ua_compliance_grade: string | null;
  pdf_ua_compliance_severity: string | null;
  ...
  color_contrast_score: number | null;
  color_contrast_grade: string | null;
  color_contrast_severity: string | null;
...
CREATE TABLE IF NOT EXISTS publist_cache (
  ...
  pdf_ua_compliance_score INTEGER,
  pdf_ua_compliance_grade TEXT,
  pdf_ua_compliance_severity TEXT,
  ...
  color_contrast_grade TEXT,
  color_contrast_severity TEXT,
...
export function initCache(cacheDir?: string): Database.Database {
  ...
  try {
    db.exec("ALTER TABLE publist_cache ADD COLUMN summary TEXT");
  } catch {}
  try {
...
export function upsertResult(
  ...
  const catMap: Record<
    string,
    { score: number | null; grade: string | null; severity: string | null }
  > = {};
...
export function getAllSuccessful(db: Database.Database): CachedRow[] {
  return db
    .prepare("SELECT * FROM publist_cache WHERE status = ? ORDER BY title ASC")
    .all("success") as CachedRow[];
}

export function getErrorCount(db: Database.Database): number {
  const row = db
    .prepare("SELECT COUNT(*) as cnt FROM publist_cache WHERE status = ?")
