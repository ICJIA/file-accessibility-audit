...
export function generateAssessment(
  ...
  if (passingPct >= 50) {
    return {
      ...
      action:
        "Focus remediation efforts on D and F files first, then address C-grade files to bring the majority into compliance.",
    };
  }
  ...
  if (failingPct >= 60) {
    return {
      ...
      action:
        'A systematic remediation effort is recommended. Start with the most recent and most-accessed publications, focusing on D and F grades. Click "View Full Analysis" on any file for specific remediation steps.',
    };
  }
  ...
  return {
    ...
    action:
      "Prioritize D and F files for remediation. Consider establishing accessibility review as part of the publication workflow to prevent future issues.",
  };
}

const RESEARCHHUB_CUTOFF = "2019-01-01";

export function splitByEra(rows: CachedRow[]): {
  recent: CachedRow[];
  legacy: CachedRow[];
} {
