...
describe("authMiddleware — PAT branch", () => {
  ...
  it("returns 401 for an unrecognised Bearer fap_ token", () => {
    const req = makeReq({
      headers: { authorization: "Bearer fap_deadbeefdeadbeefdeadbeefdeadbeef" },
    });
    const res = makeRes();
    const next = makeNext();
...
describe("POST /api/tokens — validation", () => {
  it("rejects a PAT-authenticated minting attempt with 403", async () => {
    ...
    const req = makeReq({
      body: { name: "cli" },
      user: { email: "u@illinois.gov", authMethod: "pat" },
    });
    const res = makeRes();
    const next = makeNext();
  ...
  it("creates a token and returns it in the response body (201)", async () => {
    ...
    const req = makeReq({
      body: { name: "filecap-cli" },
      user: { email: "u@illinois.gov", authMethod: "session" },
    });
    const res = makeRes();
    const next = makeNext();
