...
describe("validateUrlForFetch", () => {
  it("throws SafeFetchError(malformed_url) for a disallowed host", () => {
    expect(() => validateUrlForFetch(new URL("https://example.com/a.pdf"))).toThrowError(
      SafeFetchError,
    );
  });
...
describe("validateUrlPublic (privileged)", () => {
  it("allows any public http(s) URL", () => {
    expect(() => validateUrlPublic(new URL("https://example.com/a.pdf"))).not.toThrow();
  });
  it("still rejects non-http(s) schemes", () => {
    expect(() => validateUrlPublic(new URL("ftp://example.com/a.pdf"))).toThrowError(
      SafeFetchError,
    );
  });
});
