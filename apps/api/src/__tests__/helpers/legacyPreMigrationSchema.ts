/**
 * Faithful reproduction of the PRE-migration-runner apps/api/src/db/sqlite.ts
 * bootstrap logic (the exact SQL + probe-then-ALTER structure that ran on
 * every server boot before C3's numbered-migration runner existed), copied
 * verbatim from `git show HEAD:apps/api/src/db/sqlite.ts` at the commit
 * immediately before migrations.ts was introduced. Parameterized on a `db`
 * instance instead of module-level `import.meta` side effects, so a test can
 * build a "legacy" database — one with every current column already
 * present (an old server booted this code enough times to run every
 * backfill) but `user_version` still at SQLite's default of 0, since this
 * code never touched it.
 *
 * DO NOT "clean up" or refactor this file to match migrations.ts — its
 * entire value is being a byte-faithful copy of what the OLD code actually
 * ran, not a hand-approximation of it. If sqlite.ts's pre-migration history
 * is ever needed again, `git log -- apps/api/src/db/sqlite.ts` has it.
 */
import type Database from "better-sqlite3";

type DB = InstanceType<typeof Database>;

export function buildLegacyPreMigrationSchema(db: DB): void {
