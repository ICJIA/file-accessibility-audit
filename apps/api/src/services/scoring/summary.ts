...
export function generateSummary(
  ...
  if (conformance.status === "incomplete") {
    return `Automated analysis could not fully complete for this PDF (it may be encrypted, damaged, or unsupported), so no WCAG conformance verdict could be reached. The partial ${score}/100 score reflects only the checks that ran — a full manual accessibility review is required.`;
  }
  ...
  if (grade === "A") {
    return `This PDF scored ${score}/100 (grade ${grade}) and cleared every automated WCAG check across all ${applicable.length} assessed categories — a strong result. This is not a determination of conformance: confirm color contrast and the correctness of alt text, headings, and reading order in a manual review before publishing.`;
  }

  if (grade === "B") {
    return `This PDF scored ${score}/100 (grade ${grade}) and cleared the automated WCAG checks, with minor issues remaining — ${clean.length} of ${applicable.length} categories are fully issue-free. Review the findings below, then confirm with a manual check before publishing.`;
  }
  ...
  if (critical.length > 0 && moderate.length > 0) {
    ...
    return `This PDF scored ${score}/100 (grade ${grade}). Automated checks found no single confirmed WCAG failure, but ${critical.length} categor${critical.length === 1 ? "y has" : "ies have"} critical issues (${criticalNames}) and ${moderate.length} ${moderate.length === 1 ? "has" : "have"} moderate issues (${moderateNames}). Address these before publishing.`;
  }

  if (critical.length > 0) {
    const criticalNames = critical.map((c) => c.label).join(", ");
    return `This PDF scored ${score}/100 (grade ${grade}). ${critical.length} categor${critical.length === 1 ? "y has" : "ies have"} critical accessibility issues: ${criticalNames}. These must be addressed before publishing.`;
  }

  if (moderate.length > 0) {
    const moderateNames = moderate.map((c) => c.label).join(", ");
    return `This PDF scored ${score}/100 (grade ${grade}). ${moderate.length} categor${moderate.length === 1 ? "y has" : "ies have"} moderate accessibility issues: ${moderateNames}. These should be addressed to improve accessibility.`;
  }

  return `This PDF scored ${score}/100 (grade ${grade}), with room for improvement in ${applicable.length - clean.length} of ${applicable.length} categories. Review the findings below before publishing.`;
}
