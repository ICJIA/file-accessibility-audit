...
function logEvent(
  ...
  email: string,
  req: Request,
  extra?: { filename?: string; score?: number; grade?: string },
) {
...
router.post("/request", authRequestLimiter, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      res
        .status(503)
        .json({ error: "Unable to send verification email. Please try again in a few minutes." });
      return;
    }
...
router.post("/verify", authVerifyLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!row) {
      res
        .status(400)
        .json({ error: "No valid verification code found. Please request a new one." });
      return;
    }
    ...
    if (row.attempts >= maxAttempts) {
      ...
      res
        .status(400)
        .json({ error: "Maximum verification attempts exceeded. Please request a new code." });
      return;
    }
    ...
    const isDevBypass =
      !isProduction && process.env.ALLOW_DEV_OTP_BYPASS === "true" && otp === "000000";
