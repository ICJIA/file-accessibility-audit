...
router.post("/tokens", authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  if (req.user?.authMethod === "pat") {
    res.status(403).json({
      error:
        "Personal access tokens cannot be used to mint other tokens. Sign in with your browser to manage tokens.",
    });
    return;
  }
  ...
  const existing = db
    .prepare("SELECT COUNT(*) as count FROM access_tokens WHERE email = ? AND revoked_at IS NULL")
